...
"<br/><div id='uploadstatus'>"),
    ...
    datasourcetemplate: _.template("<form><h3>Create Data Source</h3>" +
        "<div class='simpleConnection'>" +
            "<label for='connname'>Name:</label>" +
            "<input type='text' name='connname' value='<%= conn.connectionname %>' " + "<% if(conn.id){%>" + "disabled" + "<%}%>" + "/><br/>" +
            "<label for='drivertype'>Connection Type:</label>" +
            "<select name='drivertype' class='drivertype'><option value='MONDRIAN'>Mondrian</option><option value='XMLA'>XMLA</option></select><br/>" +
            "<label for='jdbcurl'>URL:</label>" +
