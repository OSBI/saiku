...
public class OlapQueryService implements Serializable {
  ...
  public void cancel( String queryName ) {
    try {
      //			System.out.println("Cancel: ID " + Thread.currentThread().getId() + " Name: " + Thread.currentThread()
      // .getName());
      IQuery q = getIQuery( queryName );
      q.cancel();
    } catch ( Exception e ) {
  ...
  public CellDataSet execute( String queryName, ICellSetFormatter formatter ) {
    ...
    try {
      //			System.out.println("Execute: ID " + Thread.currentThread().getId() + " Name: " + Thread.currentThread()
      // .getName());
      ...
      // we could do a check if query.getTotalFunctions() actually includes a total function and if not dont execute
      // the following
      if ( QueryType.QM.equals( query.getType() ) && formatter instanceof FlattenedCellSetFormatter ) {
        ...
        for ( int index = 0; index < 2; index++ ) {
          ...
          builder =
            new TotalsListsBuilder( selectedMeasures, aggregators, cellSet, axisInfos[ index ], axisInfos[ second ] );
  ...
  public List<SimpleCubeElement> getResultMetadataMembers( String queryName, boolean preferResult, String dimensionName,
    ...
    if ( cs == null || !preferResult || members.size() == 0 || levels.size() == 1 ) {
      members = olapDiscoverService
        .getLevelMembers( query.getSaikuCube(), dimensionName, hierarchyName, levelName, searchString, searchLimit );
    }
    return members;
  }
  ...
  public ResultSet explain( String queryName ) {
    ...
    try {
      ...
      final OlapConnection con =
        olapDiscoverService.getNativeConnection( getQuery( queryName ).getCube().getConnectionName() );
